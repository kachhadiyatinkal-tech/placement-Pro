...
const AddTPO = async (req, res) => {
  ...
  try {
    ...
    const newUser = new Users({ first_name: req.body.first_name, email: req.body.email,
       number: req.body.number, password: hashPassword, role: "tpo" });
    await newUser.save();
...
const AddManagement = async (req, res) => {
  ...
  try {
    ...
    const newUser = new Users({ first_name: req.body.first_name, email: req.body.email, number: req.body.number, password: hashPassword, role: "management" });
    await newUser.save();
    return res.json({ msg: "User Created!" });
  } catch (error) {
