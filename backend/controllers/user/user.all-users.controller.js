...
const AllUsersLen = async (req, res) => {
  try {
    ...
    const tpoUsers = (await User.find({ role: "tpo" })).length;
    const managementUsers = (await User.find({ role: "management" })).length;
    const superUsers = (await User.find({ role: "admin" })).length;
