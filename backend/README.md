...
- `POST /api/v1/user/forgot-password` - To request password reset
- `POST /api/v1/user/verify-reset-token` - To verify reset token
- `POST /api/v1/user/reset-password` - To reset password with token
...
The system includes a complete forgot password functionality:

### Frontend Pages
- `/forgot-password` - Page to request password reset
- `/reset-password` - Page to reset password with token

### API Endpoints
- `POST /api/v1/user/forgot-password` - Request password reset email
- `POST /api/v1/user/verify-reset-token` - Verify reset token validity
- `POST /api/v1/user/reset-password` - Reset password with valid token

### Email Configuration
To enable email sending for password resets, configure these environment variables in `.env`:
...
**Note**: For Gmail, use an "App Password" instead of your regular password. For development/testing, the reset URL is logged to console instead of sending emails.

### Testing
Run the test script: `node test-forgot-password.js`
